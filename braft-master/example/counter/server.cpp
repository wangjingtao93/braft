...
// Implements（工具，实现） Closure（关闭，终止） which encloses（装入，围绕） RPC stuff（东西）
...
// Implementation（实例） of example::Counter as a braft::StateMachine.
...
public:
    ...
    // Impelements（执行） Service methods
    void fetch_add(const FetchAddRequest* request,
                   CounterResponse* response,
                   google::protobuf::Closure* done) {
        brpc::ClosureGuard done_guard(done);
        // Serialize（连载，使连续，序列化） request to the replicated write-ahead-log so that all the
        // peers in the group receive this request as well.
        // Notice that _value can't be modified（修改） in this routine（程序） otherwise it
        // will be inconsistent（不一致） with others in this group.
